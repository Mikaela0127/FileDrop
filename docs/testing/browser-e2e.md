...
same-origin API requests and the reserved `uploads.example.test` PUT. The
fixtures are intentionally fake and deterministic. They must never be replaced
with a real passphrase, cookie, share token, bucket name, presigned URL, or cloud
credential in a committed test.
